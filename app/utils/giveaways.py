...
def build_giveaway_result_post(ga, winner) -> str:
    parts = []
    if ga["title"]:
...
def build_giveaway_ended_post(ga, winner) -> str:
    parts = []
    if ga["title"]:
        parts.append(f"🎁 <b>{ga['title']}</b>\n")
    parts.append(f"Приз: <b>{ga['prize']}</b>")
...
async def run_giveaway(
    ...
    winner_id = weighted_pick_users(eligible) if eligible else None
    await repo.finish_giveaway(int(ga["id"]), winner_id)
    winner = None
    winner_tg = None
    if winner_id:
        await repo.add_win(winner_id)
        winner = await repo.get_user_by_id(winner_id)
        ...
        if winner and winner_tg:
            try:
                await bot.send_message(
                    ...
                    text=(
                        f"🎉 Поздравляем! Вы стали победителем розыгрыша #{ga['id']}!\n"
                        f"Приз: <b>{ga['prize']}</b>\n\n"
                        "Свяжитесь с администратором для получения приза."
                    ),
                )
                logger.info("Giveaway #%s: notified winner %s", ga["id"], winner_tg)
            except Exception as e:
                logger.error("Giveaway #%s: failed to notify winner %s: %s", ga["id"], winner_tg, e)
    if ga["channel_id"] and ga["channel_msg_id"]:
        ...
        try:
            await bot.edit_message_text(
                chat_id=channel_id,
                message_id=msg_id,
                text=build_giveaway_ended_post(ga, winner),
            )
            ...
            try:
                await bot.send_message(
                    chat_id=channel_id,
                    text=build_giveaway_result_post(ga, winner),
